...
class CubePicker:
    def __init__(self, serial_port=None, baud=1_000_000, camera_index=0, yolo_onnx_path=None, coco_names_path=None):
        # --- Hardware handles ---
        self.mc = None
        self.cap = None
    ...
    def detect_objects(self, img):
        ...
        for (left, top, width, height, score, class_id) in detections:
            # Get class name (fallback to id if no names file)
            if self.coco_classes and 0 <= class_id < len(self.coco_classes):
                name = self.coco_classes[class_id]
            else:
                name = f"class_{class_id}"

            # Draw box + center (use a neutral color)
            cv2.rectangle(img, (left, top), (left + width, top + height), (0, 255, 255), 2)
            cx, cy = left + width // 2, top + height // 2
            cv2.circle(img, (cx, cy), 3, (255, 255, 255), -1)
            cv2.putText(img, f"{name}", (left, max(0, top - 6)),
    ...
    def _box_color_label(self, img_hsv, box):
        """
        Decide which of your 4 HSV color ranges dominates inside 'box'.
        Returns one of {"blue","green","yellow","red"} or None if no clear winner.
        """
        x, y, w, h = box
        x0, y0 = max(0, x), max(0, y)
        x1, y1 = min(img_hsv.shape[1], x + w), min(img_hsv.shape[0], y + h)
        if x1 <= x0 or y1 <= y0:
            return None

        roi = img_hsv[y0:y1, x0:x1]
        area = roi.shape[0] * roi.shape[1]
        if area < 2000:  # ignore tiny boxes
            return None

        best_name, best_pixels = None, 0
        for name, (lower, upper) in self.HSV.items():
        ...
        # Require at least 10% of the ROI to be that color to avoid noise
        return best_name if best_pixels >= 0.10 * area else None
