...
class cube_picker():
    ...
    def grasp(self,x,y):
        x, y = y, x #Reversing the coordinates
        print(f"Attempting to grasp at coords ({x},{y})")

        print("Moving to general grab point")
        ...
        print("Moving to actual grab point")
...
key = None
while (key!=27) and picker.clicked_point == None:    
    _, frame = cap.read()
    ...
    if init:
        ...
        cv2.setMouseCallback("frame", picker.click_event)
        key = cv2.waitKey(1)

cap.release()
cv2.destroyAllWindows()

x = float(picker.clicked_point[0])
y = float(picker.clicked_point[1])
